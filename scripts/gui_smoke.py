...
def main() -> int:
    ...
    multi_controller = GameController()
    first_slot = multi_controller.add_engine_slot(fake_path, command=[sys.executable, str(fake_path)])
    second_slot = multi_controller.add_engine_slot(fake_path, command=[sys.executable, str(fake_path)])
    expect(first_slot is not None and second_slot is not None, "multiple fake engine slots start")
    pump(multi_controller, lambda: all(slot.ready for slot in multi_controller.engine_slots))
    multi_controller.set_side_player(chess.WHITE, first_slot.slot_id)
    multi_controller.set_side_player(chess.BLACK, second_slot.slot_id)
    multi_controller.set_play_search_mode("nodes")
    multi_controller.set_node_limit(10)
    pump(multi_controller, lambda: len(multi_controller.board.move_stack) >= 2, timeout=5.0)
    expect(len(multi_controller.board.move_stack) >= 2, "engine-vs-engine slots can play both colors")
    expect("[Event " in multi_controller.move_history_text(), "move history is exported as PGN")
    multi_controller.set_play_mode(False)
    multi_controller.shutdown()
    ...
    controller.set_play_search_mode("nodes")
    controller.set_node_limit(1500)
    expect(controller.play_search_mode == "nodes", "controller switches engine reply mode to nodes")
    expect(controller.node_limit == 1500, "controller stores the configured node limit")
    ...
    expect(len(controller.board.move_stack) >= 2, "engine reply is applied after a human move in nodes mode")
    expect("1." in controller.move_history_text(), "PGN text includes SAN move numbers")

    controller.new_game()
    controller.set_play_search_mode("clock")
    controller.set_clock(1, 1)
    expect(controller.play_search_mode == "clock", "controller switches engine reply mode to clock")
    expect(controller.clock_base_ms == 60000 and controller.clock_increment_ms == 1000, "controller stores the configured clock")
    result = controller.make_user_move_uci("d2d4")
    expect(result.status == "applied", "legal human move is applied in clock mode")
    pump(controller, lambda: len(controller.board.move_stack) >= 2, timeout=10.0)
    expect(len(controller.board.move_stack) >= 2, "engine reply is applied after a human move in clock mode")
